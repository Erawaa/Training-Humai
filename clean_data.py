...
def clean_marca():
    ...
    sql_query = f'SELECT IdProducto, Nombre FROM `{PROJECT_AND_DATASET}.precios_azucar` WHERE IdMarca = -1'
    query = client.query(sql_query)
    result = query.result()

    sql_marcas = f'SELECT IdMarca, Nombre FROM `{PROJECT_AND_DATASET}.marcas`'
